...
function migrate() {
  ...
  if (!varCols.includes('auto_paused')) {
    db.exec('ALTER TABLE variations ADD COLUMN auto_paused INTEGER DEFAULT 0');
  }
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_vv_var ON variation_versions(variation_id);
  `);
